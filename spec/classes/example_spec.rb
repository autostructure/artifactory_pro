    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context 'artifactory_pro class with simple license key' do
          ...
          it { is_expected.to contain_class('artifactory').that_comes_before('Class[artifactory_pro::config]') }
          it { is_expected.to contain_class('artifactory_pro::config').that_comes_before('Class[artifactory_pro::post_config]') }
          it { is_expected.to contain_class('artifactory_pro::config') }
          it { is_expected.to contain_class('artifactory_pro::post_config') }
          it { is_expected.to contain_class('artifactory::service').that_subscribes_to('Class[artifactory_pro::config]') }
          it { is_expected.to contain_class('artifactory::service').that_subscribes_to('Class[artifactory_pro::post_config]') }
